/*
  ...
  5. Hold sticks centered and still during the first ~0.5s
     ...
     runs. A serial message announces when it's done. This is the
     only calibration step that needs your attention.
  6. RANGE self-calibration (fixes high "Circle Error" on gamepad
     testers): no manual step needed. The stick's real min/max
     travel is learned automatically and quietly during normal
     play - the first time you push a direction further than
     before, that becomes the new known edge for that axis. If a
     bad reading ever throws this off, hold BACK+START for 2s to
...
// sticks (left/right thumbsticks), not LT/RT. Lowered from 180:
// if your stick's real travel (see calibrateRange() serial output)
// is only a few hundred raw counts from center to edge, a large
// deadzone eats most of that range and makes movement feel like
// it jumps straight from 0 to near-max. Raise this back up only
// if you see idle jitter with the stick centered.
...
// Per-axis inversion, toggled live with a button combo (see loop()) -
// no reflash needed. Defaults to not inverted. Resets to false on
// power cycle since it's RAM-only; say the word if you want this
// remembered across reboots and I'll add flash (Preferences) storage.
bool invertLX = false;
bool invertLY = false;
bool invertRX = false;
bool invertRY = false;
// Calibrated per-axis extremes. Start with a reasonable default
// (works okay out of the box for most joystick modules) and then
// AUTO-EXPAND toward the real extremes automatically the first
// time you move the stick that far during normal play - see
// expandRange() in loop(). No manual calibration ritual needed.
int minLX = 400,  maxLX = 3700;
int minLY = 400,  maxLY = 3700;
int minRX = 400,  maxRX = 3700;
int minRY = 400,  maxRY = 3700;
...
// we bother sending a new report for the sticks. Kept small so
// small, deliberate movements still get reported - too high a
// value here is what makes the stick feel like an on/off button
// instead of smooth analog travel.
const int16_t AXIS_CHANGE_THRESHOLD = 40;
...
// Called every loop iteration with the current raw readings.
// Whenever a reading goes past what's been seen before, that
// becomes the new known extreme for that axis. This means the
// range calibrates itself automatically just from normal play -
// no dedicated calibration step, no timer, no watching Serial.
// It only ever WIDENS the range (never narrows), so it can't
// suddenly make the stick feel more sensitive than before.
void expandRange(int lx, int ly, int rx, int ry) {
...
// Reset the range back to the safe starting defaults, in case
// expandRange() ever picks up a bad one-off spike (e.g. a loose
// wire) and you want a clean slate. Hold BACK+START for 2s.
void resetRange() {
  minLX = 400; maxLX = 3700;
  minLY = 400; maxLY = 3700;
  minRX = 400; maxRX = 3700;
  minRY = 400; maxRY = 3700;
  Serial.println("Stick range reset to defaults - it'll re-expand as you play.");
}
...
void setup() {
  ...
  Serial.println("Calibrating center - leave sticks untouched a moment...");
  calibrateSticks();
  Serial.println("Ready. Stick range will fine-tune itself as you play.");

  Serial.println("Advertising...");
}

void loop() {
  // ---------- Hotkey: hold BACK+START for 2s to reset stick range ----------
  // Only needed if a bad reading ever widened the range incorrectly
  // (e.g. a loose wire spike). Normally you'll never need this -
  // the range just quietly expands to fit your stick's real travel
  // the first time you move it that far.
  static unsigned long comboHeldSince = 0;
  bool backHeld  = (digitalRead(PIN_BACK)  == LOW);
  bool startHeld = (digitalRead(PIN_START) == LOW);
  if (backHeld && startHeld) {
    if (comboHeldSince == 0) comboHeldSince = millis();
    if (millis() - comboHeldSince > 2000) {
      resetRange();
      comboHeldSince = 0;
    }
  } else {
  ...
  // ---------- Hotkey: BACK + D-pad direction toggles axis invert ----------
  // Tap (not hold) while holding BACK:
  //   D-pad Up/Down    -> invert Left  Y
  //   D-pad Left/Right -> invert Left  X
  // Combine with R3 held at the same time to target the RIGHT stick
  // instead of the left, e.g. BACK+R3+D-pad Up -> invert Right Y.
  // Edge-detected so each physical press toggles exactly once, and
  // normal D-pad presses are suppressed while BACK is held so they
  // don't also register as menu input to the host.
  static bool lastDpadUp = false, lastDpadDown = false;
  static bool lastDpadLeft = false, lastDpadRight = false;
  bool r3Held = (digitalRead(PIN_R3) == LOW);
  bool dpadUp    = (digitalRead(PIN_DPAD_UP)    == LOW);
  bool dpadDown  = (digitalRead(PIN_DPAD_DOWN)  == LOW);
  bool dpadLeft  = (digitalRead(PIN_DPAD_LEFT)  == LOW);
  bool dpadRight = (digitalRead(PIN_DPAD_RIGHT) == LOW);

  if (backHeld) {
    if (dpadUp && !lastDpadUp) {
      if (r3Held) { invertRY = !invertRY; Serial.print("Right Y invert: "); Serial.println(invertRY ? "ON" : "OFF"); }
      else        { invertLY = !invertLY; Serial.print("Left Y invert: ");  Serial.println(invertLY ? "ON" : "OFF"); }
    }
    ...
    if (dpadLeft && !lastDpadLeft) {
      if (r3Held) { invertRX = !invertRX; Serial.print("Right X invert: "); Serial.println(invertRX ? "ON" : "OFF"); }
      else        { invertLX = !invertLX; Serial.print("Left X invert: ");  Serial.println(invertLX ? "ON" : "OFF"); }
    }
  ...
  if (bleGamepad.isConnected()) {
    ...
    for (int i = 0; i < numButtons; i++) {
      // While BACK is held, D-pad presses are reserved for the
      // invert-toggle hotkey above and shouldn't also reach the host.
      bool isDpad = (buttons[i].pin == PIN_DPAD_UP || buttons[i].pin == PIN_DPAD_DOWN ||
                     buttons[i].pin == PIN_DPAD_LEFT || buttons[i].pin == PIN_DPAD_RIGHT);
      if (isDpad && backHeld) {
        continue;
      }
    ...
    // ---------- Analog Sticks ----------
    // Oversampled reads. expandRange() quietly grows the known
    // min/max the first time you reach a new extreme, so the range
    // fits your stick's real travel within a few normal movements -
    // no dedicated calibration step required.
    int rawLX = readAveraged(PIN_LSTICK_X);
    int rawLY = readAveraged(PIN_LSTICK_Y);
    int rawRX = readAveraged(PIN_RSTICK_X);
    int rawRY = readAveraged(PIN_RSTICK_Y);
    expandRange(rawLX, rawLY, rawRX, rawRY);

    float nlx = normalizeAxis(rawLX, centerLX, minLX, maxLX);
    float nly = normalizeAxis(rawLY, centerLY, minLY, maxLY);
    float nrx = normalizeAxis(rawRX, centerRX, minRX, maxRX);
    float nry = normalizeAxis(rawRY, centerRY, minRY, maxRY);

    // Apply live-toggled inversion (see BACK+D-pad hotkey above).
    if (invertLX) nlx = -nlx;
    if (invertLY) nly = -nly;
    if (invertRX) nrx = -nrx;
    if (invertRY) nry = -nry;
