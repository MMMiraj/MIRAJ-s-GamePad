...
void loop() {
  if (bleGamepad.isConnected()) {
    ...
    // setAxes order is actually (x, y, z, rZ, rX, rY, slider1, slider2)
    // per the BleGamepad library signature. Right stick Y is enabled
    // via setIncludeRzAxis(true), so it must be passed in the rZ slot
    // (4th argument), not the rY slot (6th) - rX/rY are disabled in
    // the config above, which is why the right stick's Y axis wasn't
    // registering even though the wiring and raw ADC read tested fine.
