...
// stick around its full outer edge during calibration. Raised
// from 4000 - a quick circular swipe tends to undershoot the true
// physical extremes, which is what makes 50% throw read as 100%
// (normalizeAxis divides by a max that's smaller than reality).
const unsigned long RANGE_CAL_MS = 7000;
...
void calibrateRange() {
  ...
  Serial.println("Push EACH stick fully to every extreme and HOLD");
  Serial.println("briefly: full up, full down, full left, full right,");
  Serial.println("and all 4 corners. A quick circular swipe tends to");
  Serial.println("undershoot the true edges - that's what makes half");
  Serial.println("throw read as full throw. Be deliberate. Live readout:");
  ...
  while (millis() - start < RANGE_CAL_MS) {
    ...
    // Live feedback every 500ms - watch these numbers as you push.
    // If a number stops growing while you're still pushing that
    // direction, you've found the true mechanical limit there.
    if (millis() - lastPrint > 500) {
      lastPrint = millis();
      unsigned long remaining = (RANGE_CAL_MS - (millis() - start)) / 1000;
      Serial.print(remaining); Serial.print("s | LX[");
      Serial.print(minLX); Serial.print(","); Serial.print(maxLX);
      Serial.print("] LY["); Serial.print(minLY); Serial.print(",");
      Serial.print(maxLY); Serial.print("] RX["); Serial.print(minRX);
      Serial.print(","); Serial.print(maxRX); Serial.print("] RY[");
      Serial.print(minRY); Serial.print(","); Serial.print(maxRY);
      Serial.println("]");
    }
  }
  ...
  // Safety floor: if a side genuinely wasn't pushed far enough during
  // the window, fall back to a value based on THIS stick's own
  // measured span rather than a blind guess, so the fallback can't
  // itself be the thing that's too narrow (or too wide) for your
  // specific hardware.
  const int MIN_SANE_RANGE = 300;
  bool rangeTooSmall = false;
  if ((maxLX - centerLX) < MIN_SANE_RANGE) { maxLX = centerLX + max(MIN_SANE_RANGE, maxLX - centerLX); rangeTooSmall = true; }
  if ((centerLX - minLX) < MIN_SANE_RANGE) { minLX = centerLX - max(MIN_SANE_RANGE, centerLX - minLX); rangeTooSmall = true; }
  if ((maxLY - centerLY) < MIN_SANE_RANGE) { maxLY = centerLY + max(MIN_SANE_RANGE, maxLY - centerLY); rangeTooSmall = true; }
  if ((centerLY - minLY) < MIN_SANE_RANGE) { minLY = centerLY - max(MIN_SANE_RANGE, centerLY - minLY); rangeTooSmall = true; }
  if ((maxRX - centerRX) < MIN_SANE_RANGE) { maxRX = centerRX + max(MIN_SANE_RANGE, maxRX - centerRX); rangeTooSmall = true; }
  if ((centerRX - minRX) < MIN_SANE_RANGE) { minRX = centerRX - max(MIN_SANE_RANGE, centerRX - minRX); rangeTooSmall = true; }
  if ((maxRY - centerRY) < MIN_SANE_RANGE) { maxRY = centerRY + max(MIN_SANE_RANGE, maxRY - centerRY); rangeTooSmall = true; }
  if ((centerRY - minRY) < MIN_SANE_RANGE) { minRY = centerRY - max(MIN_SANE_RANGE, centerRY - minRY); rangeTooSmall = true; }

  if (rangeTooSmall) {
    Serial.println("NOTE: at least one direction was under 300 raw counts");
    Serial.println("from center - that direction's true mechanical limit");
    Serial.println("was used as-is rather than being widened arbitrarily.");
  }

  Serial.println("Hold BACK+START for 2s any time to redo this without");
...
void loop() {
  // ---------- Recalibration hotkey: hold BACK+START for 2s ----------
  // Lets you redo range calibration live while testing on
  // gamepadla.com without reflashing. Checked even if BLE isn't
  // connected yet, so it works right after power-on too.
  ...
  if (backHeld && startHeld) {
    ...
    if (millis() - comboHeldSince > 2000) {
      Serial.println("Recalibration hotkey triggered.");
      calibrateSticks();
      calibrateRange();
      comboHeldSince = 0;
    }
