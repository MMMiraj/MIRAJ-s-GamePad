...
void calibrateRange() {
  ...
  // Safety floor: if the stick wasn't actually rotated far enough
  // during the window above (missed the window, moved too slowly,
  // etc.), the captured min/max end up very close to center. That
  // makes normalizeAxis() see almost the whole range as "past the
  // deadzone", so tiny movements jump straight to ~100% output -
  // this is what makes the stick feel like an on/off button rather
  // than smooth analog travel. If a side's range looks too small,
  // widen it back out to a sane default instead of trusting it.
  const int MIN_SANE_RANGE = 400;
  bool rangeTooSmall = false;
  if ((maxLX - centerLX) < MIN_SANE_RANGE || (centerLX - minLX) < MIN_SANE_RANGE) { maxLX = centerLX + 1800; minLX = centerLX - 1800; rangeTooSmall = true; }
  if ((maxLY - centerLY) < MIN_SANE_RANGE || (centerLY - minLY) < MIN_SANE_RANGE) { maxLY = centerLY + 1800; minLY = centerLY - 1800; rangeTooSmall = true; }
  if ((maxRX - centerRX) < MIN_SANE_RANGE || (centerRX - minRX) < MIN_SANE_RANGE) { maxRX = centerRX + 1800; minRX = centerRX - 1800; rangeTooSmall = true; }
  if ((maxRY - centerRY) < MIN_SANE_RANGE || (centerRY - minRY) < MIN_SANE_RANGE) { maxRY = centerRY + 1800; minRY = centerRY - 1800; rangeTooSmall = true; }

  if (rangeTooSmall) {
    Serial.println("WARNING: calibrated range was too narrow on at least");
    Serial.println("one axis (stick probably wasn't rotated far enough");
    Serial.println("during the window). Falling back to a default range");
    Serial.println("on that axis - re-flash and rotate the sticks firmly");
    Serial.println("around their full edge next time for a proper fit.");
  }
}
